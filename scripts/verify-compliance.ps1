...
<#
.SYNOPSIS
    Architecture & Quality Standards Compliance Verification Script for dotnet-caching.
.DESCRIPTION
...
Write-Host "  Repository: dotnet-caching" -ForegroundColor Cyan
Write-Host "==================================================" -ForegroundColor Cyan
...
# -----------------------------------------------------------------------------
# Directory.Build.props Standards & icon.png Invariants
# -----------------------------------------------------------------------------
Write-Host "`n[Gate: Directory.Build.props Standards] Validating Directory.Build.props standards & icon.png..." -ForegroundColor Yellow
$propsErrors = 0
...
if (-not (Test-Path $iconPath)) {
    ...
    if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
    if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("icon.png missing from root directory") } else { $Violations++ } }
    if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
    $propsErrors++
}
...
if (-not (Test-Path $bldPropsPath)) {
    ...
    $propsContent = Get-Content $bldPropsPath -Raw -Encoding utf8
    if ($propsContent -notmatch "<ImplicitUsings>disable</ImplicitUsings>") {
        ...
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("Directory.Build.props must specify <ImplicitUsings>disable</ImplicitUsings>.") } else { $Violations++ } }
        if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
        $propsErrors++
    }
    if ($propsContent -notmatch "<PackageProjectUrl>https://ericksonlopez\.dev/caching</PackageProjectUrl>") {
        Write-Host "  ❌ PackageProjectUrl must be set to https://ericksonlopez.dev/caching." -ForegroundColor Red
        if (Get-Variable -Name "violations" -Scope 0 -ErrorAction SilentlyContinue) { $violations++ }
        if (Get-Variable -Name "Violations" -Scope 0 -ErrorAction SilentlyContinue) { if ($Violations -is [System.Collections.IList]) { $Violations.Add("PackageProjectUrl must be set to https://ericksonlopez.dev/caching.") } else { $Violations++ } }
        if (Get-Variable -Name "FailedChecks" -Scope 0 -ErrorAction SilentlyContinue) { $FailedChecks++ }
        $propsErrors++
    }
}
...
if ($propsErrors -eq 0) {
    Write-Host "  ✅ Directory.Build.props standards (ImplicitUsings=disable, PackageProjectUrl) and icon.png verified." -ForegroundColor Green
}
